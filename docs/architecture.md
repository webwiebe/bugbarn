...
5. Storage persists the processed issue, event, and facet rows in SQLite.
6. The API server serves read endpoints for the UI, and the browser client renders issue lists, issue detail, event detail, and recent live events from those read APIs.
...
- `internal/api`: HTTP server and route dispatch. It owns the read/query/live endpoints and delegates ingest to the ingest handler.
...
- `internal/storage`: SQLite schema, inserts, issue grouping, event persistence, facet storage, and read APIs.
...
- `internal/api` owns the HTTP server surface, including `GET /api/v1/issues`, `GET /api/v1/issues/{id}`, `GET /api/v1/issues/{id}/events`, `GET /api/v1/events/{id}`, and `GET /api/v1/live/events`.
